    ...
    config = function()
        ...
        local on_attach = function(client, bufnr)
            ...
            vim.keymap.set("n", "K", function() vim.lsp.buf.hover() end,
                { buffer = bufnr, remap = false, desc = "Hover information" })
            vim.keymap.set("n", "[d", function() vim.diagnostic.goto_next() end,
                { buffer = bufnr, remap = false, desc = "Goto next diagnostic" })
            vim.keymap.set("n", "]d", function() vim.diagnostic.goto_prev() end,
                { buffer = bufnr, remap = false, desc = "Goto prev diagnostic" })

            vim.keymap.set("n", "<leader>ld", function() vim.diagnostic.open_float() end,
                { buffer = bufnr, remap = false, desc = "Open floating diagnostic" })
            ...
            vim.keymap.set("n", "<leader>lf", function() vim.lsp.buf.format({ formatting_options = { tabSize = 4 } }) end,
                { buffer = bufnr, remap = false, desc = "Format buffer" })

            vim.keymap.set({ "i", "n" }, "<C-h>", function() vim.lsp.buf.signature_help() end,
                { buffer = bufnr, remap = false, desc = "Signature help" })
            vim.keymap.set("n", "<leader>li",
                function() vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled({})) end,
                { remap = false, desc = "Inlay hints" })
            if client.name == "clangd" then
                vim.keymap.set({ "i", "n" }, "<C-t>", "<cmd>ClangdSwitchSourceHeader<cr>",
                    { buffer = bufnr, remap = false, desc = "Switch between source/header" })
            end
        end
